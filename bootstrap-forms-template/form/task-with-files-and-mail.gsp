...
<html xmlns:w="w" xmlns:f="f" xmlns:g="g" xmlns:form="form"  xmlns:xforms="xforms">
	...
	<body>
		<mgzl:formwithmail action="submit" title="${form.title}" explanation="${form.explanationMessage}" name="${form.name}" process="${form.workflow.name}" type="task" task="*{task}" enctype="multipart/form-data">
			</div>
			<table>						
