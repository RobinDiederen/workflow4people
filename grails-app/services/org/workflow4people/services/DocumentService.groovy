...
class DocumentService implements InitializingBean {
    ...
    def storeDocument(document) throws DocumentException {
    	...
    	Document.withTransaction { status ->
			...
			def workflowName=header.workflowName?.text()!="" ? header.workflowName.text() : header.documentType.text()
	    		
				processInstance = executionService.startProcessInstanceByKey(workflowName,variableMap)
